...
const Explore = () => {
  ...
  const handleAddToFavourites = useCallback(async (courseId: string) => {
    try {
      await dispatch(addToFavourites({ courseId })).unwrap();
      setCourses(prev =>
  ...
  const CourseCard = React.memo(
    ({
      ...
      return (
        <div className="bg-white rounded-2xl border hover:shadow-2xl transition flex flex-col overflow-hidden">
                ...
                {courses.length > 0 ? (
                  courses.map((course) => (
                    <CourseCard
                      ...
                      onAddFav={handleAddToFavourites}
                      onRemoveFav={handleRemoveFromFavourites}
                    />
                  ))
                ) : (
